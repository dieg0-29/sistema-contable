from sqlalchemy import BigInteger, Boolean, Date, ForeignKey, Numeric, SmallInteger, String, Text, TIMESTAMP
...
class Cuenta(Base):
    ...
    cuenta_creacion: Mapped[str] = mapped_column(TIMESTAMP(timezone=True))
...
class Transaccion(Base):
    ...
    tsc_creacion: Mapped[str] = mapped_column(TIMESTAMP(timezone=True))
...
class DetalleTransaccion(Base):
    ...
    id_tsc: Mapped[int] = mapped_column(ForeignKey("transacciones.id_tsc", ondelete="CASCADE"), nullable=False)
    id_cuenta: Mapped[int] = mapped_column(ForeignKey("cuentas.id_cuenta", ondelete="RESTRICT"), nullable=False)
    ...
    detalle_creacion: Mapped[str] = mapped_column(TIMESTAMP(timezone=True))